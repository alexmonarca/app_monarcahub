...
import { supabase as supabaseClient, partnersSupabase } from "@/lib/supabaseClient";
...
const DSFIT_ALLOWED_USER_ID = "1a6a75c4-3f75-4706-ab1f-b46aeb5d7d23";
const DSFIT_ALLOWED_EMAIL = "dsfittabuleiro@dsfit.com.br";
...
const WEBHOOK_FORGOT_PASSWORD_URL = "https://webhook.monarcahub.com/webhook/esqueci";
const PARTNERS_LEADS_TABLE = "partner_program_leads";
const HOTMART_AFFILIATE_URL = "https://affiliate.hotmart.com/affiliate-recruiting/view/7809I103050471";
...
const parseMoneyToInteger = (value) => {
...
const PartnersHotmartRedirectModal = ({ isOpen, onClose, onConfirm }) => {
  if (!isOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden">
        ...
        <div className="p-6 md:p-8">
          <h3 className="text-xl font-bold text-white">Redirecionamento para ambiente seguro</h3>
          <p className="mt-3 text-sm text-gray-300 leading-relaxed">
            Você será redirecionado para um ambiente seguro da Hotmart, onde poderá concluir o cadastro de parceiro.
          </p>

          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <Button type="button" onClick={onClose} variant="secondary" className="sm:flex-1">
              Cancelar
            </Button>
            <Button type="button" onClick={onConfirm} variant="primary" className="sm:flex-1">
              Continuar na Hotmart <ExternalLink className="w-4 h-4" />
            </Button>
          </div>
        </div>
      </div>
...
const LoginStepsModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-md relative overflow-hidden">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-white bg-gray-800 p-1 rounded-full z-10"
          aria-label="Fechar"
        ...
        <div className="p-6 md:p-8">
          <h2 className="text-xl font-bold text-white">Acesso com parceiro comercial</h2>
          <p className="text-xs text-gray-400 mt-1">Passo {step} de 2</p>
          ...
          {step === 1 ? (
            <div className="mt-5">
              <InputGroup
                label="Código informado pelo parceiro"
                value={partnerCode}
            ...
            <div className="mt-5">
              <label className="flex items-center gap-2 text-sm text-gray-300 mb-3">
                <input
                  type="checkbox"
                  checked={alreadyPaidFee}
                  onChange={(e) => onAlreadyPaidFeeChange(e.target.checked)}
                  disabled={loading}
                />
                Já paguei a taxa
              </label>
              ...
              {!alreadyPaidFee && (
                <InputGroup
                  label="Valor da taxa de implementação"
                  value={implementationFee}
                  onChange={(e) => onImplementationFeeChange(e.target.value)}
                  placeholder="Ex.: 150"
                  required
                  disabled={loading}
                  helpText="Use apenas valores em múltiplos de R$ 50 (50, 100, 150...)."
                />
              )}
...
const SignupInviteModal = ({
  isOpen,
  isPartnersRoute = false,
  loading,
  error,
  success,
  form,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-xl relative overflow-hidden">
        ...
        <form onSubmit={onSubmit} className="p-6 md:p-8">
          <h2 className="text-2xl font-bold text-white">Teste grátis com convite</h2>
          <p className="text-sm text-gray-400 mt-1 mb-6">
            Você receberá um convite para conhecer a plataforma após um de nossos parceiros comerciais falar com você.
          </p>
          ...
          {isPartnersRoute && (
            <>
              <InputGroup
                label="Qual atividade atual — já atuou com vendas?"
                value={form.currentSalesActivity}
                onChange={(e) => onChange("currentSalesActivity", e.target.value)}
                placeholder="Descreva sua atividade e experiência com vendas"
                multiline
                required
                disabled={loading}
              />
              <InputGroup
                label="Como nos encontrou — quem indicou nosso programa de parceiros?"
                value={form.referralSource}
                onChange={(e) => onChange("referralSource", e.target.value)}
                placeholder="Conte como nos encontrou e, se houver, quem indicou"
                multiline
                required
                disabled={loading}
              />
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button type="button" onClick={onClose} variant="secondary" className="sm:flex-1" disabled={loading}>
              Fechar
            </Button>
            <Button type="submit" variant="primary" className="sm:flex-1" disabled={loading}>
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (isSignUp && !result.data.session) setError("Cadastro realizado! Verifique seu email.");
        ...
        if (!isSignUp) {
          const expectedCode = String(requiredPartnerCode || "")
            .trim()
            .toUpperCase();

          if (!expectedCode) {
            await supabaseClient.auth.signOut();
            setError("Validação do código não encontrada. Volte e refaça o acesso com parceiro.");
            return;
          }

          const { data: gymConfig, error: gymConfigError } = await supabaseClient
            .from("gym_configs")
            .select("code_access")
            .eq("user_id", result.data.user.id)
            .maybeSingle();

          if (gymConfigError) throw gymConfigError;

          const registeredCode = String(gymConfig?.code_access || "")
            .trim()
            .toUpperCase();

          if (!registeredCode || registeredCode !== expectedCode) {
            await supabaseClient.auth.signOut();
            setError("Código do parceiro não confere com o cadastro desta conta.");
            return;
          }
        }
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-2xl flex flex-col items-center">
        ...
        <div className="mt-6 text-center">
          ...
          {!isSignUp && (
            <button onClick={onForgotPasswordClick} className="block mt-3 mx-auto text-sm text-gray-300 hover:underline">
              Esqueci Senha
            </button>
          )}
        </div>
...
const ForgotPasswordScreen = ({ onBackToLogin, onBackToLanding }) => {
  const [email, setEmail] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(WEBHOOK_FORGOT_PASSWORD_URL, {
        method: "POST",
        mode: "cors",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok) throw new Error("Não foi possível enviar sua solicitação agora. Tente novamente.");

      setSuccess("Solicitação enviada! Se encontrarmos sua conta, enviaremos um email.");
      setEmail("");
      setPhone("");
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-2xl flex flex-col items-center">
        ...
        <div className="text-center mb-8 w-full">
          <p className="text-gray-400 mt-2">Informe seu email e, se quiser, o telefone para recuperação.</p>
        </div>
        ...
        {success && (
          <div className="bg-green-500/10 text-green-400 p-3 rounded-lg mb-4 text-sm text-center border border-green-500/20 w-full">
            {success}
          </div>
        )}
        ...
        <button onClick={onBackToLogin} className="mt-6 text-sm text-orange-400 hover:underline">
          Voltar para login
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <div className="rounded-2xl border border-orange-500/20 bg-gray-900 p-8 shadow-xl">
          ...
          <div className="mt-8 overflow-hidden rounded-xl border border-gray-700 bg-gray-950">
            <div className="aspect-video w-full">
              <iframe
                className="h-full w-full"
                src="https://www.youtube.com/embed/9Ij9Lj44v9A"
                title="Vídeo do Programa de Parceiros IARA"
                loading="lazy"
...
export default function App() {
  ...
  const [showLoginStepsModal, setShowLoginStepsModal] = useState(false);
  const [loginStep, setLoginStep] = useState(1);
  const [partnerCode, setPartnerCode] = useState("");
  const [validatedPartnerCode, setValidatedPartnerCode] = useState("");
  const [implementationFee, setImplementationFee] = useState("");
  const [alreadyPaidFee, setAlreadyPaidFee] = useState(false);
  const [loginStepLoading, setLoginStepLoading] = useState(false);
  const [loginStepError, setLoginStepError] = useState("");
  ...
  const [signupInviteError, setSignupInviteError] = useState("");
  const [signupInviteSuccess, setSignupInviteSuccess] = useState("");
  const [signupInviteForm, setSignupInviteForm] = useState({
    name: "",
    businessName: "",
    email: "",
    phone: "",
    currentSalesActivity: "",
    referralSource: "",
  });
  ...
  const isPartnersConfigured = Boolean(env.partnersSupabaseUrl && env.partnersSupabaseAnonKey);
  ...
  const resetLoginStepsModal = () => {
    setShowLoginStepsModal(false);
    setLoginStep(1);
    setPartnerCode("");
    setImplementationFee("");
    setAlreadyPaidFee(false);
    setLoginStepError("");
  ...
  const handleOpenLoginFlow = () => {
    setLoginStepError("");
    setValidatedPartnerCode("");
    setShowLoginStepsModal(true);
  };

  const handleLoginNextStep = async () => {
    setLoginStepError("");

    if (loginStep === 1) {
      const code = String(partnerCode || "").trim();
      if (!code) {
        setLoginStepError("Informe o código enviado pelo parceiro.");
        return;
      }

      const partnersClient = isPartnersConfigured && partnersSupabase ? partnersSupabase : supabaseClient;

      setLoginStepLoading(true);
      try {
        const { data, error } = await partnersClient
          .from(PARTNERS_LEADS_TABLE)
          .select("id")
          .eq("code", code)
          .limit(1);
        ...
        if (!Array.isArray(data) || data.length === 0) {
          setLoginStepError("Código inválido. Confira e tente novamente.");
          return;
        }

        setValidatedPartnerCode(code);
        setLoginStep(2);
      } catch (_error) {
        setLoginStepError("Não foi possível validar o código agora. Tente novamente.");
      } finally {
        setLoginStepLoading(false);
      }
      return;
    ...
    if (!alreadyPaidFee) {
      const feeValue = parseMoneyToInteger(implementationFee);
      if (!feeValue) {
        setLoginStepError("Informe o valor da taxa de implementação.");
        return;
      }
      if (feeValue < 50) {
        setLoginStepError("A taxa mínima é R$ 50.");
        return;
      }
      if (feeValue % 50 !== 0) {
        setLoginStepError("Use apenas valores de 50 em 50 (50, 100, 150...).");
        return;
      }
    }

    resetLoginStepsModal();
    setAuthView("login");
  };

  const handleSignupInviteFieldChange = (field, value) => {
    setSignupInviteForm((prev) => ({ ...prev, [field]: value }));
  };

  const handleSignupInviteSubmit = async (e) => {
    e.preventDefault();
    setSignupInviteError("");
    setSignupInviteSuccess("");

    const payload = {
      type: "visitor_invite_request",
      nome: String(signupInviteForm.name || "").trim(),
      nome_negocio: String(signupInviteForm.businessName || "").trim(),
      email: String(signupInviteForm.email || "").trim(),
      telefone: normalizePhone(signupInviteForm.phone),
      atividade_atual_vendas: String(signupInviteForm.currentSalesActivity || "").trim(),
      como_nos_encontrou_indicacao: String(signupInviteForm.referralSource || "").trim(),
      origem: isPublicPartnersRoute ? "parceiros_criar_conta_gratis" : "landing_teste_gratis",
    };

    if (!payload.nome || !payload.nome_negocio || !payload.email || !payload.telefone) {
      setSignupInviteError("Preencha todos os campos para solicitar o convite.");
      return;
    }

    if (isPublicPartnersRoute && (!payload.atividade_atual_vendas || !payload.como_nos_encontrou_indicacao)) {
      setSignupInviteError("Preencha também as perguntas do programa de parceiros.");
      return;
    }
    ...
    try {
      ...
      try {
        const response = await fetch(WEBHOOK_PARTNERS_URL, {
          method: "POST",
          mode: "cors",
          headers: { "Content-Type": "application/json" },
        ...
        // Continua para fallback no-cors abaixo
      }

      if (!sent) {
        await fetch(WEBHOOK_PARTNERS_URL, {
          method: "POST",
          mode: "no-cors",
      ...
      setSignupInviteSuccess("Solicitação enviada. Um parceiro comercial entrará em contato.");
      setSignupInviteForm({
        name: "",
        businessName: "",
        email: "",
      ...
      setSignupInviteError("Não foi possível enviar agora. Tente novamente.");
    } finally {
  ...
  if (!session) {
    if (authView === "forgot-password") {
      return (
        <ForgotPasswordScreen onBackToLogin={() => setAuthView("login")} onBackToLanding={() => setAuthView("landing")} />
      );
    }
    ...
    if (authView === "landing") {
      if (isPublicPartnersRoute) {
        return (
          <>
            <PublicPartnersPage
              onOpenLogin={handleOpenLoginFlow}
              onOpenSignup={() => setShowSignupInviteModal(true)}
              onBackToLanding={() => navigate("/")}
            />

            <LoginStepsModal
              isOpen={showLoginStepsModal}
              step={loginStep}
              partnerCode={partnerCode}
              implementationFee={implementationFee}
              alreadyPaidFee={alreadyPaidFee}
              loading={loginStepLoading}
              error={loginStepError}
              onPartnerCodeChange={setPartnerCode}
              onImplementationFeeChange={setImplementationFee}
              onAlreadyPaidFeeChange={(checked) => {
                setAlreadyPaidFee(checked);
                if (checked) setImplementationFee("");
              }}
              onClose={resetLoginStepsModal}
              onNext={handleLoginNextStep}
              onBack={() => {
                setLoginStep(1);
                setLoginStepError("");
              }}
            ...
            <SignupInviteModal
              isOpen={showSignupInviteModal}
              isPartnersRoute={isPublicPartnersRoute}
              loading={signupInviteLoading}
              error={signupInviteError}
              success={signupInviteSuccess}
              form={signupInviteForm}
              onChange={handleSignupInviteFieldChange}
              onClose={() => {
                setShowSignupInviteModal(false);
                setSignupInviteError("");
                setSignupInviteSuccess("");
              }}
              onSubmit={handleSignupInviteSubmit}
            />
          </>
        );
      }
      ...
      return (
        <>
          <LandingPage onOpenLogin={handleOpenLoginFlow} onOpenSignup={() => setShowSignupInviteModal(true)} />
    ...
    return (
      <AuthScreen
        ...
        requiredPartnerCode={validatedPartnerCode}
        onBackToLanding={() => {
          setValidatedPartnerCode("");
          setAuthView("landing");
...
function Dashboard({ session }) {
  ...
  const [isPartnerAccessLoading, setIsPartnerAccessLoading] = useState(true);
  const [hasPartnerProgramAccess, setHasPartnerProgramAccess] = useState(false);
  ...
  useEffect(() => {
    ...
    if (next === "partners" && !isPartnerAccessLoading && !hasPartnerProgramAccess) {
      navigate("/", { replace: true });
      _setActiveTab("dashboard");
      return;
    }
    ...
    _setActiveTab(next);
  }, [location.pathname, hasPartnerProgramAccess, isPartnerAccessLoading, navigate]);
  ...
  const [isDsfitAlunoSearching, setIsDsfitAlunoSearching] = useState(false);
  const [dsfitAlunoError, setDsfitAlunoError] = useState("");
  const [dsfitAlunoSuccess, setDsfitAlunoSuccess] = useState("");
  const [dsfitAlunoSearchError, setDsfitAlunoSearchError] = useState("");
  const [dsfitAlunoSearchTerm, setDsfitAlunoSearchTerm] = useState("");
  const [dsfitAlunoSearchResults, setDsfitAlunoSearchResults] = useState([]);
  const [hasDsfitAlunoSearchRun, setHasDsfitAlunoSearchRun] = useState(false);
  const [dsfitAlunoForm, setDsfitAlunoForm] = useState({
    codigo: "",
    nome_completo: "",
    plano: "",
    periodicidade: "",
  ...
  const [isDsfitDatabaseOpen, setIsDsfitDatabaseOpen] = useState(false);
  ...
  const [isPartnersHotmartModalOpen, setIsPartnersHotmartModalOpen] = useState(false);
  ...
  const partnersFaqItems = [
    ...
    },
    {
      ...
      answer:
        "Você define e cobra a taxa inicial (opcional), ficando com 70% do valor que você cobrar, conforme o porte da empresa: Pequeno (R$50–150), Médio (R$151–300) e Grande (R$301–3 mil).",
    },
    {
  ...
  useEffect(() => {
    ...
    const loadPartnerAccess = async () => {
      setIsPartnerAccessLoading(true);
      try {
        ...
        if (!orFilters.length) {
          if (!mounted) return;
          setHasPartnerProgramAccess(false);
          return;
        }
        ...
        const approved = Array.isArray(data) && data.length > 0;
        if (!mounted) return;
        setHasPartnerProgramAccess(Boolean(approved));
      } catch (error) {
        console.error("Erro ao validar acesso ao programa de parceiros:", error);
        if (mounted) setHasPartnerProgramAccess(false);
      } finally {
        if (mounted) setIsPartnerAccessLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const loadDsfitAccess = async () => {
      setIsDsfitAccessLoading(true);
      try {
        ...
        const matchesAllowedUser =
          sessionId === DSFIT_ALLOWED_USER_ID && sessionEmail === DSFIT_ALLOWED_EMAIL;

        if (!matchesAllowedUser) {
          if (mounted) setHasDsfitStudentsAccess(false);
          return;
        }
        ...
        const hasAccess =
          profileId === DSFIT_ALLOWED_USER_ID && profileEmail === DSFIT_ALLOWED_EMAIL;

        if (mounted) setHasDsfitStudentsAccess(hasAccess);
      } catch (error) {
        console.error("Erro ao validar acesso ao banco de alunos DSFit:", error);
        if (mounted) setHasDsfitStudentsAccess(false);
      } finally {
        if (mounted) setIsDsfitAccessLoading(false);
      }
    };
  ...
  const handleDsfitAlunoSubmit = async (e) => {
    e.preventDefault();
    setDsfitAlunoError("");
    setDsfitAlunoSuccess("");

    if (!hasDsfitStudentsAccess) {
      setDsfitAlunoError("Sem permissão para cadastrar alunos.");
      return;
    }

    setIsDsfitAlunoSaving(true);

    try {
      const payload = {
        codigo: dsfitAlunoForm.codigo ? Number(dsfitAlunoForm.codigo) : null,
        nome_completo: String(dsfitAlunoForm.nome_completo || "").trim() || null,
        plano: String(dsfitAlunoForm.plano || "").trim() || null,
        periodicidade: String(dsfitAlunoForm.periodicidade || "").trim() || null,
        inicio: dsfitAlunoForm.inicio || null,
        fim: dsfitAlunoForm.fim || null,
        status: String(dsfitAlunoForm.status || "").trim() || null,
        valor: parseCurrencyToNumber(dsfitAlunoForm.valor),
        valor_cobranca: parseCurrencyToNumber(dsfitAlunoForm.valor_cobranca),
        celular_limpo: String(dsfitAlunoForm.celular_limpo || "").trim() || null,
        link_contrato: String(dsfitAlunoForm.link_contrato || "").trim() || null,
        unidade: String(dsfitAlunoForm.unidade || "").trim() || "Tabuleiro",
      };

      const { error } = await supabaseClient.from("dsfit_alunos").insert(payload);
      if (error) throw error;

      setDsfitAlunoSuccess("Aluno cadastrado com sucesso.");
      setDsfitAlunoForm((prev) => ({
        ...prev,
        codigo: "",
        nome_completo: "",
        plano: "",
        ...
        celular_limpo: "",
        link_contrato: "",
        unidade: prev.unidade || "Tabuleiro",
      }));
    } catch (error) {
      setDsfitAlunoError(error?.message || "Não foi possível cadastrar o aluno agora.");
    } finally {
      setIsDsfitAlunoSaving(false);
    }
  };

  const handleDsfitAlunoSearch = async (e) => {
    e.preventDefault();
    setDsfitAlunoSearchError("");
    setHasDsfitAlunoSearchRun(true);

    if (!hasDsfitStudentsAccess) {
      setDsfitAlunoSearchError("Sem permissão para consultar alunos.");
      setDsfitAlunoSearchResults([]);
      return;
    }

    const rawTerm = String(dsfitAlunoSearchTerm || "").trim();
    if (rawTerm.length < 2) {
      setDsfitAlunoSearchError("Digite pelo menos 2 caracteres para pesquisar.");
      setDsfitAlunoSearchResults([]);
      return;
    }
    ...
    if (!safeTerm) {
      setDsfitAlunoSearchError("Digite um termo válido para pesquisar.");
      setDsfitAlunoSearchResults([]);
      return;
    }

    setIsDsfitAlunoSearching(true);

    try {
      const numericTerm = safeTerm.replace(/\D/g, "");
      const searchFilter =
        numericTerm.length >= 3
          ? `nome_completo.ilike.%${safeTerm}%,plano.ilike.%${safeTerm}%,status.ilike.%${safeTerm}%,codigo.eq.${Number(numericTerm)},celular_limpo.ilike.%${numericTerm}%`
          : `nome_completo.ilike.%${safeTerm}%,plano.ilike.%${safeTerm}%,status.ilike.%${safeTerm}%,celular_limpo.ilike.%${safeTerm}%`;

      const { data, error } = await supabaseClient
        .from("dsfit_alunos")
        .select("id, codigo, nome_completo, plano, status, celular_limpo, unidade, created_at")
        .or(searchFilter)
        .order("created_at", { ascending: false })
        .limit(30);
      ...
      setDsfitAlunoSearchResults(Array.isArray(data) ? data : []);
    } catch (error) {
      setDsfitAlunoSearchError(error?.message || "Não foi possível consultar os alunos agora.");
      setDsfitAlunoSearchResults([]);
    } finally {
      setIsDsfitAlunoSearching(false);
    }
  };
  ...
  const updateBranchPhone = (id, val) =>
    setGymData((prev) => ({ ...prev, branches: prev.branches.map((b) => (b.id === id ? { ...b, phone: val } : b)) }));
  ...
  const handleOpenPartnersHotmartModal = () => {
    setIsPartnersHotmartModalOpen(true);
  };

  const handleConfirmPartnersHotmartRedirect = () => {
    setIsPartnersHotmartModalOpen(false);
    window.open(HOTMART_AFFILIATE_URL, "_blank", "noopener,noreferrer");
  };
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
        ...
        return (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-right-4 duration-300">
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card title="Informações Básicas" icon={Dumbbell}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InputGroup
                    ...
                    helpText="Como a IA deve chamar sua marca nas mensagens."
                  />
                  <InputGroup
                    label="Telefone / WhatsApp"
                    value={gymData.phone}
                ...
                <div className="pt-4 border-t border-gray-700 mt-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {gymData.branches.map((branch) => (
                      <div key={branch.id} className="space-y-2 border border-orange-500/20 rounded-lg p-3 bg-gray-900/40">
                        ...
                        <input
                          className="w-full bg-gray-900 border border-orange-500/30 rounded-lg p-3 text-gray-100"
                          value={branch.phone || ""}
                          placeholder="Telefone da Filial (ex: (12)12345-6789)"
                          onChange={(e) => updateBranchPhone(branch.id, e.target.value)}
                        />
                      </div>
                    ))}
                  </div>
              ...
              {!isDsfitAccessLoading && hasDsfitStudentsAccess && (
                <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-xl overflow-hidden">
                  <div
                    className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-700/50 transition-colors"
                    onClick={() => setIsDsfitDatabaseOpen(!isDsfitDatabaseOpen)}
                  >
                    <div className="flex items-center gap-3">
                      <Users className="w-6 h-6 text-orange-400" />
                      <h3 className="text-lg font-semibold text-white">Banco de Dados de Clientes / Alunos</h3>
                    </div>
                    {isDsfitDatabaseOpen ? (
                      <ChevronUp className="w-5 h-5 text-gray-400" />
                    ) : (
                  ...
                  {isDsfitDatabaseOpen && (
                    <div className="p-6 pt-2 border-t border-gray-700">
                      <p className="text-sm text-gray-300 mb-5">Cadastre aqui o aluno e detalhes como plano e contato</p>

                      <div className="mb-5 p-4 bg-gray-900/50 border border-gray-700 rounded-lg">
                        <p className="text-sm text-gray-300 mb-3">Pesquisar Clientes / Alunos (tabela dsfit_alunos)</p>
                        <form onSubmit={handleDsfitAlunoSearch} className="flex flex-col md:flex-row gap-2">
                          <input
                            className="flex-1 bg-gray-900 border border-orange-500/30 rounded-lg p-3 text-gray-100"
                            placeholder="Buscar por nome, plano, status, código ou celular"
                            value={dsfitAlunoSearchTerm}
                            onChange={(e) => setDsfitAlunoSearchTerm(e.target.value)}
                          />
                          <Button type="submit" disabled={isDsfitAlunoSearching}>
                            {isDsfitAlunoSearching ? "Pesquisando..." : "Pesquisar"}
                          </Button>
                        </form>

                        {dsfitAlunoSearchError && (
                          <p className="mt-3 text-sm text-red-300 bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                            {dsfitAlunoSearchError}
                          </p>
                        )}

                        {hasDsfitAlunoSearchRun && !dsfitAlunoSearchError && (
                          <div className="mt-3 space-y-2">
                            {dsfitAlunoSearchResults.length === 0 ? (
                              <p className="text-sm text-gray-400 border border-gray-700 rounded-lg p-3">Nenhum cliente/aluno encontrado.</p>
                            ) : (
                              dsfitAlunoSearchResults.map((aluno) => (
                                <div key={aluno.id} className="border border-gray-700 rounded-lg p-3 bg-gray-800/60">
                                  <p className="text-sm text-white font-medium">{aluno.nome_completo || "Sem nome"}</p>
                                  <p className="text-xs text-gray-300 mt-1">
                                    Código: {aluno.codigo ?? "-"} • Plano: {aluno.plano || "-"} • Status: {aluno.status || "-"}
                                  </p>
                                  <p className="text-xs text-gray-400 mt-1">
                                    Celular: {aluno.celular_limpo || "-"} • Unidade: {aluno.unidade || "-"}
                                  </p>
                                </div>
                              ))
                            )}
                      ...
                      <form onSubmit={handleDsfitAlunoSubmit} className="space-y-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <InputGroup
                        label="Código"
                        type="number"
                        value={dsfitAlunoForm.codigo}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, codigo: e.target.value }))}
                      />
                      <InputGroup
                        label="Nome Completo"
                        value={dsfitAlunoForm.nome_completo}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, nome_completo: e.target.value }))}
                      />
                      <InputGroup
                        label="Plano"
                        value={dsfitAlunoForm.plano}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, plano: e.target.value }))}
                      />
                      <InputGroup
                        label="Periodicidade"
                        value={dsfitAlunoForm.periodicidade}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, periodicidade: e.target.value }))}
                      />
                      <InputGroup
                        label="Início"
                        type="date"
                        value={dsfitAlunoForm.inicio}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, inicio: e.target.value }))}
                      />
                      <InputGroup
                        label="Fim"
                        type="date"
                        value={dsfitAlunoForm.fim}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, fim: e.target.value }))}
                      />
                      <InputGroup
                        label="Status"
                        value={dsfitAlunoForm.status}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, status: e.target.value }))}
                      />
                      <InputGroup
                        label="Valor"
                        type="text"
                        placeholder="Ex: 149,90"
                        value={dsfitAlunoForm.valor}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, valor: e.target.value }))}
                      />
                      <InputGroup
                        label="Valor Cobrança"
                        type="text"
                        placeholder="Ex: 149,90"
                        value={dsfitAlunoForm.valor_cobranca}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, valor_cobranca: e.target.value }))}
                      />
                      <InputGroup
                        label="Celular Limpo"
                        value={dsfitAlunoForm.celular_limpo}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, celular_limpo: e.target.value }))}
                      />
                      <InputGroup
                        label="Link do Contrato"
                        value={dsfitAlunoForm.link_contrato}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, link_contrato: e.target.value }))}
                      />
                      <InputGroup
                        label="Unidade"
                        value={dsfitAlunoForm.unidade}
                        onChange={(e) => setDsfitAlunoForm((prev) => ({ ...prev, unidade: e.target.value }))}
                      />
                    </div>

                    {dsfitAlunoError && (
                      <p className="mt-2 text-sm text-red-300 bg-red-500/10 border border-red-500/20 rounded-lg p-3">{dsfitAlunoError}</p>
                    )}
                    {dsfitAlunoSuccess && (
                      <p className="mt-2 text-sm text-green-300 bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                        {dsfitAlunoSuccess}
                      </p>
                    )}

                        <div className="pt-3">
                          <Button type="submit" disabled={isDsfitAlunoSaving}>
                            {isDsfitAlunoSaving ? "Salvando aluno..." : "Salvar aluno"}
                          </Button>
                        </div>
                      </form>
                    </div>
      ...
      case "partners":
        if (isPartnerAccessLoading) {
          return (
            <div className="max-w-4xl mx-auto animate-in fade-in duration-300">
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 text-gray-300 flex items-center gap-3">
                <Loader2 className="w-5 h-5 animate-spin text-orange-400" /> Validando acesso ao programa de parceiros...
              </div>
            </div>
          );
        }

        if (!hasPartnerProgramAccess) {
          return (
            <div className="max-w-4xl mx-auto animate-in fade-in duration-300">
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-lg text-sm text-red-200">Acesso negado.</div>
            </div>
          );
        }
            ...
            <div className="bg-gray-900 border border-orange-500/20 rounded-2xl p-8 shadow-xl">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-3 text-sm">
                ...
                <div className="rounded-xl border border-gray-700 bg-gray-950 p-4">
                  ...
                  <p className="text-gray-300 mt-1">R$ 301 a R$ 3 mil</p>
                </div>
              </div>

              <p className="mt-8 text-sm text-orange-200">
                O Parceiro ganha <strong>70% de toda taxa de configuração</strong> - isso significa que você fica com a grande maior parte.
              </p>

              <button
                type="button"
                onClick={() => setShowPartnersFaq((prev) => !prev)}
                className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-6 py-3 font-semibold text-white hover:bg-orange-600 transition-colors"
              >
              ...
              {showPartnersFaq && (
                <div className="mt-6 space-y-3 border-t border-gray-700/80 pt-6">
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row sm:items-center gap-3">
                    <button
                      type="button"
                      onClick={handleOpenPartnersHotmartModal}
                      className="inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-6 py-3 font-semibold text-white hover:bg-orange-600 transition-colors"
                    >
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans flex flex-col md:flex-row">
      ...
      <PartnersHotmartRedirectModal
        isOpen={isPartnersHotmartModalOpen}
        onClose={() => setIsPartnersHotmartModalOpen(false)}
        onConfirm={handleConfirmPartnersHotmartRedirect}
      />
    </div>
  );
}
